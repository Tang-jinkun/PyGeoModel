...
Make completed multi-radar tasks use the existing right-side `WorkbenchInspector` for result inspection and full artifact downloads. Tighten the task-center history-row layout so terminal tasks do not reserve an empty progress column.

## Scope

- Reuse the existing `WorkbenchInspector`; do not add a second drawer, modal, or output panel.
...
- Reuse the inspector's existing output-files section so every existing output descriptor is available as a download.
...
2. The application resolves the task from the already loaded multi-radar history, selects it as the active inspector context, and loads or reuses its output descriptors.
3. The existing inspector renders multi-radar metrics and the shared file list using that context.
4. A layer action loads the aggregate artifacts through the established adapter; a file action leaves the inspector focused on its existing output-files section and uses existing download paths.
