...
def test_visibility_volume_uses_terrain_height_limit_and_negative_elevation(
    tmp_path: Path,
) -> None:
    dem_path = tmp_path / "relief-dem.tif"
    min_height_path = tmp_path / "relief-minimum-visible-height.tif"
    shape = (21, 21)
    ...
    payload = CoverageRequest.model_validate(
        {
            "dem_id": "dem_relief_volume",
            "radar": {"lon": 79.0, "lat": 31.5, "height_m": 10},
            "coverage": {"max_range_m": 1_000, "scan_mode": "omni"},
            "advanced": {
                "voxel_max_height_m": 500,
    ...
    radar_z = 510
    assert volume.vertices[:, 2].min() < radar_z - 50
    assert volume.vertices[:, 2].max() <= 1_025
    unique_z = numpy.unique(numpy.round(volume.vertices[:, 2], 6))
    assert numpy.diff(unique_z).max() <= 40
