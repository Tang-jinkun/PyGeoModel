...
def validate_recon_vehicle_task_id(task_id: str) -> None:
    if not RECON_VEHICLE_TASK_ID_PATTERN.fullmatch(task_id):
        raise AppError("INVALID_TASK_ID", "Recon vehicle task id contains unsupported characters.", status_code=400)


def create_recon_vehicle_task(payload: ReconVehicleCoverageRequest) -> ReconVehicleCoverageTaskStatus:
    now = utc_now()
    task_id = f"recon_vehicle_task_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
    task = ReconVehicleCoverageTaskStatus(
        task_id=task_id,
    ...
    save_recon_vehicle_task(task, payload)
    return task
...
def get_recon_vehicle_task(task_id: str) -> ReconVehicleCoverageTaskStatus:
    with _task_lock(task_id):
        path = _task_path(task_id)
        if not path.exists():
            raise AppError("TASK_NOT_FOUND", f"Recon vehicle task '{task_id}' was not found.", status_code=404)
...
def list_recon_vehicle_tasks() -> list[ReconVehicleCoverageTaskSummary]:
    tasks: list[ReconVehicleCoverageTaskSummary] = []
    for path in settings.tasks_dir.glob("recon_vehicle_task_*.json"):
        try:
            data = _read_task_data(path)
...
def mark_recon_vehicle_running(task_id: str, message: str, progress: int = 10) -> None:
    with _task_lock(task_id):
...
def mark_recon_vehicle_finished(
    task_id: str,
    metrics: ReconVehicleCoverageMetrics,
    outputs: ReconVehicleCoverageOutputs,
    output_files: list[ReconVehicleOutputFile],
...
def mark_recon_vehicle_failed(task_id: str, message: str) -> None:
    with _task_lock(task_id):
        task = get_recon_vehicle_task(task_id)
        task.status = "failed"
        task.progress = 100
...
def recover_interrupted_recon_vehicle_tasks() -> int:
    recovered = 0
    for path in settings.tasks_dir.glob("recon_vehicle_task_*.json"):
        with _task_lock(path.stem):
            ...
            _save_task_unlocked(task, parse_recon_vehicle_request(payload))
            recovered += 1
    return recovered


def parse_recon_vehicle_request(payload) -> ReconVehicleCoverageRequest | None:
    if not payload:
        return None
    try:
        return ReconVehicleCoverageRequest.model_validate(payload)
    except Exception:
        return None
...
def _save_task_unlocked(task: ReconVehicleCoverageTaskStatus, payload: ReconVehicleCoverageRequest | None = None) -> None:
...
def _parse_task_record(data: dict, detail: bool) -> tuple[ReconVehicleCoverageTaskStatus | ReconVehicleCoverageTaskSummary, dict | None]:
    task_data = data.get("task")
    if not isinstance(task_data, dict):
        raise AppError("TASK_RECORD_CORRUPT", "Recon vehicle task data is invalid.", status_code=500)
    task = ReconVehicleCoverageTaskStatus.model_validate(task_data) if detail else ReconVehicleCoverageTaskSummary.model_validate(task_data)
