...
def validate_artillery_task_id(task_id: str) -> None:
    if not ARTILLERY_TASK_ID_PATTERN.fullmatch(task_id):
        raise AppError("INVALID_TASK_ID", "Artillery task id contains unsupported characters.", status_code=400)


def create_artillery_task(payload: ArtilleryCoverageRequest) -> ArtilleryCoverageTaskStatus:
    now = utc_now()
    task_id = f"artillery_task_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
    task = ArtilleryCoverageTaskStatus(
        task_id=task_id,
    ...
    save_artillery_task(task, payload)
    return task
...
def get_artillery_task(task_id: str) -> ArtilleryCoverageTaskStatus:
    with _task_lock(task_id):
        path = _task_path(task_id)
        if not path.exists():
            raise AppError("TASK_NOT_FOUND", f"Artillery task '{task_id}' was not found.", status_code=404)
...
def list_artillery_tasks() -> list[ArtilleryCoverageTaskSummary]:
    tasks: list[ArtilleryCoverageTaskSummary] = []
    for path in settings.tasks_dir.glob("artillery_task_*.json"):
        try:
            data = _read_task_data(path)
...
def mark_artillery_running(task_id: str, message: str, progress: int = 10) -> None:
    with _task_lock(task_id):
...
def mark_artillery_finished(
    task_id: str,
    metrics: ArtilleryCoverageMetrics,
    outputs: ArtilleryCoverageOutputs,
    output_files: list[ArtilleryOutputFile],
...
def mark_artillery_failed(task_id: str, message: str) -> None:
    with _task_lock(task_id):
        task = get_artillery_task(task_id)
        task.status = "failed"
        task.progress = 100
...
def recover_interrupted_artillery_tasks() -> int:
    recovered = 0
    for path in settings.tasks_dir.glob("artillery_task_*.json"):
        with _task_lock(path.stem):
            ...
            _save_task_unlocked(task, parse_artillery_request(payload))
            recovered += 1
    return recovered


def parse_artillery_request(payload) -> ArtilleryCoverageRequest | None:
    if not payload:
        return None
    try:
        return ArtilleryCoverageRequest.model_validate(payload)
    except Exception:
        return None
...
def _save_task_unlocked(task: ArtilleryCoverageTaskStatus, payload: ArtilleryCoverageRequest | None = None) -> None:
...
def _parse_task_record(data: dict, detail: bool) -> tuple[ArtilleryCoverageTaskStatus | ArtilleryCoverageTaskSummary, dict | None]:
    task_data = data.get("task")
    if not isinstance(task_data, dict):
        raise AppError("TASK_RECORD_CORRUPT", "Artillery task data is invalid.", status_code=500)
    task = ArtilleryCoverageTaskStatus.model_validate(task_data) if detail else ArtilleryCoverageTaskSummary.model_validate(task_data)
