...
def validate_watchpost_task_id(task_id: str) -> None:
    if not WATCHPOST_TASK_ID_PATTERN.fullmatch(task_id):
        raise AppError("INVALID_TASK_ID", "Watchpost task id contains unsupported characters.", status_code=400)


def create_watchpost_task(payload: WatchpostDetectionRequest) -> WatchpostDetectionTaskStatus:
    now = utc_now()
    task_id = f"watchpost_task_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
    task = WatchpostDetectionTaskStatus(
        task_id=task_id,
    ...
    save_watchpost_task(task, payload)
    return task
...
def get_watchpost_task(task_id: str) -> WatchpostDetectionTaskStatus:
    with _task_lock(task_id):
        path = _task_path(task_id)
        if not path.exists():
            raise AppError("TASK_NOT_FOUND", f"Watchpost task '{task_id}' was not found.", status_code=404)
...
def list_watchpost_tasks() -> list[WatchpostDetectionTaskSummary]:
    tasks: list[WatchpostDetectionTaskSummary] = []
    for path in settings.tasks_dir.glob("watchpost_task_*.json"):
        try:
            data = _read_task_data(path)
...
def mark_watchpost_failed(task_id: str, message: str) -> None:
    with _task_lock(task_id):
        task = get_watchpost_task(task_id)
        task.status = "failed"
        task.progress = 100
...
def recover_interrupted_watchpost_tasks() -> int:
    recovered = 0
    for path in settings.tasks_dir.glob("watchpost_task_*.json"):
        with _task_lock(path.stem):
            ...
            _save_task_unlocked(task, parse_watchpost_request(payload))
            recovered += 1
    return recovered


def parse_watchpost_request(payload) -> WatchpostDetectionRequest | None:
    if not payload:
        return None
    try:
        return WatchpostDetectionRequest.model_validate(payload)
    except Exception:
        return None
...
def _save_task_unlocked(task: WatchpostDetectionTaskStatus, payload: WatchpostDetectionRequest | None = None) -> None:
...
def _parse_task_record(data: dict, detail: bool) -> tuple[WatchpostDetectionTaskStatus | WatchpostDetectionTaskSummary, dict | None]:
    task_data = data.get("task")
    if not isinstance(task_data, dict):
        raise AppError("TASK_RECORD_CORRUPT", "Watchpost task data is invalid.", status_code=500)
    task = WatchpostDetectionTaskStatus.model_validate(task_data) if detail else WatchpostDetectionTaskSummary.model_validate(task_data)
