...
def describe_output_file(task_id: str, kind: CoverageOutputKind, path: Path) -> CoverageOutputFile:
    exists = path.exists()
    spec = get_output_contract("radar").spec(kind)
    download_path = get_output_contract("radar").download_path_template.format(task_id=task_id, kind=kind)
...
def list_task_output_files(task_id: str) -> list[CoverageOutputFile]:
    return [CoverageOutputFile.model_validate(item.model_dump()) for item in get_artifact_store().list_descriptors(
        task_id, get_output_contract("radar")
    )]
