  ...
  <section class="workbench-task-center" aria-label="任务中心">
    ...
    <div class="tasks-body">
      ...
      <div v-else class="task-pane active">
        <div v-for="task in visibleMultiRadarTasks" :key="task.task_id" class="task-row" :class="{ 'is-terminal': !isMultiRadarRunning(task) }" data-multi-radar-task role="button" tabindex="0" @click="emit('select-multi-radar-task', task.task_id, 'layers')" @keydown.enter="emit('select-multi-radar-task', task.task_id, 'layers')">
